...
{
    /** Customer asked to return; waiting on admin approval (order_statuses.id). */
    public const RETURN_REQUESTED = 9;
    ...
    public function orderStatusChange(Request $request)
    {
        ...
        if (in_array((int) $order->order_stat, [7, 8, 9], true)) {
            return redirect()->route('customer.order')
                ->with('error', __('This item has already been cancelled, returned, or has a return awaiting approval.'));
        }
    ...
    public function cancel(Request $request, $id)
    {
        ...
        if (in_array((int) $details->order_stat, [7, 8, 9], true)) {
            return redirect()->route('customer.order')
                ->with('error', __('This item has already been cancelled, returned, or has a return awaiting approval.'));
        }

        // A cancellation takes effect immediately. A return is only a *request*:
        // it waits on admin approval, so stock and the seller's commission are
        // left alone until BackendOrderController::approveReturn() runs.
        $isReturn = (int) $request->order_stat === 8;
        $newStat = $isReturn ? self::RETURN_REQUESTED : 7;

        DB::transaction(function () use ($details, $order, $request, $newStat, $isReturn) {
            ...
            if (!$isReturn && $product && $product->is_manage_stock) {
                $product->quantity += $details->qty;
                $product->save();
            }

            if (!$isReturn && $product && ($details->color || $details->size)) {
            ...
            if (!$isReturn && $order->payment_status === 'paid' && $product) {
        ...
        return redirect()->route('customer.order')->with('status', $isReturn
            ? __('Your return request has been submitted. Our team will review and approve it shortly.')
