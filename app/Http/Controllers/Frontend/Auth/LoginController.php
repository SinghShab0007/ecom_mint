...
class LoginController extends Controller
{
    ...
    */
    ...
    public function login(Request $request): RedirectResponse
    {
        ...
        // Correct credentials but the address was never confirmed. Send them to
        // the OTP screen. This sits *after* the password check so an outsider
        // cannot use the login form to trigger OTP mail for someone else.
        if (is_null($user->email_verified_at)) {
            $request->session()->put('pending_email_verification_user_id', $user->id);

            return redirect()->route('customer.verification.show')
                ->with('error', __('Your email is not verified yet. Enter the OTP we sent you, or request a new one.'));
        }
