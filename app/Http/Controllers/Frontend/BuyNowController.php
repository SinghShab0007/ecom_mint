...
class BuyNowController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('customer.buynow.checkout', compact('product', 'billing', 'shipping', 'gatewayEnabled'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->payment_method === PaymentController::GATEWAY) {
            return response()->json([
    ...
    public function orderStore(Request $request, $product)
    {
        ...
        $isGateway = $request->get('payment_by') === PaymentController::GATEWAY;
        $paymentStatus = $isCod ? 'unpaid' : ($isGateway ? 'pending' : 'paid');
        $paidAmount = $isCod || $isGateway ? 0 : (float) ($request->paid_amount ?? 0);
