...
class OrderController extends Controller
{
        ...
        foreach ($records as $record) {
            ...
            $data_arr[] = array(
                "order_no" => '<a title="' . $record->cancel_note . '" href="' . $show_route . '"><span class="text-primary">' . $record->order_no . '</span>' .
                    ...
                    ' at ' . date("h:i A", strtotime($record->created_at)). ($request->order_no == $record->id ? ' <i class="fa fa-bell text-danger" aria-hidden="true"></i> ' : ' ') . '</a>',
    ...
    {
        $order_overview = $this->orderOverview();
        return view('ordermanagement::orders.return_requests', compact( 'order_overview'));
    }

    /* Process ajax request */
    /**
     * DataTable feed for the return queue.
     *
     * Returns are raised per line item, not per order, so this lists
     * order_details rather than orders - otherwise there is no id to approve.
     */
    public function returnRequestList(Request $request)
    {
        $draw = $request->get('draw');
        ...
        $query = OrderDetail::query()
            ->with(['order:id,order_no,user_first_name,user_last_name,payment_status,payment_by', 'product:id,name'])
            ->whereIn('order_stat', [9, 8]);
        ...
        if ($search !== '') {
            $query->where(function ($q) use ($search) {
                $q->whereHas('order', fn ($o) => $o->where('order_no', 'like', "%{$search}%")
        ...
        $rows = $query->orderByRaw('FIELD(order_stat, 9, 8)')
            ->orderByDesc('id')
            ->skip($start)->take($rowperpage ?: 10)->get();

        $data = [];
        foreach ($rows as $d) {
            $pending = (int) $d->order_stat === 9;
            $reason = OrderTimeline::where('order_detail_id', $d->id)
                ->whereIn('order_stat', [8, 9])->orderByDesc('id')->first();

            $actions = $pending
                ? '<button class="btn btn-sm btn-success approve-return" data-id="' . $d->id . '">'
                    . __('Approve') . '</button> '
                  . '<button class="btn btn-sm btn-danger reject-return" data-id="' . $d->id . '">'
                    . __('Reject') . '</button>'
                : '<span class="text-muted">' . __('Completed') . '</span>';

            $data[] = [
                'order_no' => '<a href="' . route(auth('seller')->user() ? 'seller.orders.show' : 'backend.orders.show', $d->order_id) . '">'
                    . e(optional($d->order)->order_no) . '</a>',
                'customer' => e(trim(optional($d->order)->user_first_name . ' ' . optional($d->order)->user_last_name)),
                'product' => e(optional($d->product)->name),
                'qty' => (int) $d->qty,
                'payment' => e(optional($d->order)->payment_by . ' / ' . optional($d->order)->payment_status),
                'reason' => e(optional($reason)->order_stat_desc) . (optional($reason)->remarks ? '<br><small class="text-muted">' . e($reason->remarks) . '</small>' : ''),
                'status' => $pending
                    ? '<span class="badge bg-warning">' . __('RETURN REQUESTED') . '</span>'
     ...
     *
     * The customer's request only flags the line as RETURN REQUESTED; the
     * inventory and commission effects happen here, once a human agrees.
     */
    public function approveReturn(Request $request)
    {
        $request->validate(['detail_id' => 'required|integer|exists:order_details,id']);

        $details = OrderDetail::findOrFail($request->detail_id);

        if ((int) $details->order_stat !== 9) {
            return response()->json(['message' => __('This item has no pending return request.')], 422);
        }

        $order = Order::findOrFail($details->order_id);

        DB::transaction(function () use ($details, $order) {
            ...
            if ($product && ($details->color || $details->size)) {
                $colorId = $details->color ? optional(Color::where('name', $details->color)->first())->id : null;
                $sizeId  = $details->size  ? optional(Size::where('name', $details->size)->first())->id   : null;
            ...
            // Reverse the seller's earnings only for money actually collected.
            ...
            OrderTimeline::create([
                ...
                'order_stat' => 8,
                'order_stat_desc' => __('Return approved by admin'),
                'order_stat_datetime' => now(),
                'remarks' => '',
            ]);

            $details->update(['order_stat' => 8]);
        });

        return response()->json(['message' => __('Return approved. Stock has been restored.')]);
    }

    /**
     * Reject a return request and put the line back to DELIVERED.
     */
    public function rejectReturn(Request $request)
    {
        ...
        DB::transaction(function () use ($details, $request) {
            OrderTimeline::create([
                'order_detail_id' => $details->id,
                'user_id' => $details->user_id,
                'product_id' => $details->product_id,
                'order_stat' => 6,
                'order_stat_desc' => __('Return rejected by admin'),
                'order_stat_datetime' => now(),
                'remarks' => $request->reason ?? '',
            ]);

            // Nothing was reversed on request, so there is nothing to undo.
            $details->update(['order_stat' => 6]);
        });

        return response()->json(['message' => __('Return request rejected.')]);
    }
