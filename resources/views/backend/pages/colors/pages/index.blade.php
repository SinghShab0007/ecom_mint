    ...
    <div class="content-body">
        <div class="container">
            <div class="main-content default-manu">
                @include('backend.pages.colors.nav')
                <!-- Tab Manu End  -->
                <!-- Tab Content Start -->
                            ...
                            <table id="colorTable" class="table p-table text-center">
                                ...
                                @foreach($colors as $color)
                                    <tr>
                                        <td>{{ $color->name }}</td>
                                        <td>{{$color->hex }}</td>
                                        <td><div class="form-switch"><input class="form-check-input status" type="checkbox" data-id="{{$color->id}}" @if($color->is_active) checked @endif></div></td>
                                        <td> 
                                            <ul>
                                                <li>
                                                    <a class="p-0 action" href="{{route('backend.colors.edit' , $color->id )}}">
                                                ...
                                                <li>
                                                    <form user="deleteForm" method="POST"
                                                          action="{{route('backend.colors.destroy',$color->id )}}">
                                                        @csrf
                                                        @method('delete')
