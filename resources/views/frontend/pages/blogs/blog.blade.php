        ...
        <div class="row">
            ...
            <div class="col-lg-8">
                <div class="mb-4">
                    ...
                    <p class="text-muted mb-0">{{ __('Stay updated with offers, product guides and news from BillMintMall.') }}</p>
                </div>
                <div class="row g-4">
                        ...
                        <div class="col-md-6">
                            <article class="card h-100 shadow-sm border-0 blog-card">
                                <a href="{{ route('frontend.blog.details', $blog->slug) }}" class="card-thumb d-block position-relative overflow-hidden">
                                    <div class="blog-color-panel w-100 d-flex align-items-center justify-content-center"
                                         style="background: {{ blogColor($blog) }};"
                                         role="img" aria-label="{{ $blog->title }}">
                                        <span class="blog-color-panel__initial">{{ blogInitial($blog) }}</span>
                                    </div>
                        ...
                        @foreach($blogs->take(5) as $recentpost)
                            <div class="blog-post-categories d-flex mb-3">
                                <a href="{{ route('frontend.blog.details', $recentpost->slug) }}" class="post-thumb me-3">
                                    <span class="blog-color-thumb rounded d-flex align-items-center justify-content-center"
                                          style="background: {{ blogColor($recentpost) }};"
                                          role="img" aria-label="{{ $recentpost->title }}">{{ blogInitial($recentpost) }}</span>
                                </a>
