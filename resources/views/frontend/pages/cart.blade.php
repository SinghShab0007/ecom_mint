                        ...
                        @foreach ($carts as $key => $cart)
                            <tr id="cart-row-{{ $key }}">
                                ...
                                <td class="table-quantity">
                                    <form>
                                        <div class="quantity">
                                            <input type="button" value="-" class="minus" data-key="{{ $key }}" data-id="{{ $cart->id }}" data-product-stock="{{ $cart->product_stock ?? '' }}">
                                            <input type="number" class="input-number qty" min="1" name="quantity" value="{{ $cart->quantity }}" onchange="syncCartQty($(this))" data-key="{{ $key }}" data-id="{{ $cart->id }}" data-product-stock="{{ $cart->product_stock ?? '' }}">
                                            <input type="button" value="+" class="plus" data-key="{{ $key }}" data-id="{{ $cart->id }}" data-product-stock="{{ $cart->product_stock ?? '' }}">
                                        </div>
                                    </form>
                                </td>
                                <td>{{ $cart->courier ?? '' }}</td>
...
<script>
     ...
     */
    /*
     * The theme's quantity() helper (public/frontend/js/index.js) already binds
     * .plus/.minus and steps the input by one. This only PERSISTS whatever value
     * the input ends up holding - it must never step the value itself, or a
     * single click counts twice.
     */
    function syncCartQty(elem) {
        var row   = elem.closest('tr');
        var input = row.find('.qty');
        var key   = input.data('key');
        var id    = input.data('id');
        var stock = parseInt(input.data('product-stock'), 10);
    ...
    /*
     * Delegated on document, so it runs AFTER the theme's element-bound handler
     * has already stepped the input - we then save the resulting value.
     */
    $(document).on('click', '.table-quantity .plus, .table-quantity .minus', function () {
        syncCartQty($(this));
    });
